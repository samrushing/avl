...
# cdef static int tree_from_list (object * list,
...
cdef int tree_from_tree(avl_node ** node,
    ...
    if low == high:
        ...
        new_node = avl.avl_new_avl_node (< void * > 0, parent)
        if not new_node:
            return -1
        address[0] = new_node
...
cdef int avl_copy_avl_node(avl_node * source_node,
    ...
    # cdef avl_node * new_node = avl.avl_new_avl_node(
    cdef avl_node * new_node
...
cdef class tree:
    ...
    def __cinit__(self, args=None, object compare_function=None):
        ...
        # self.tree = <avl.avl_tree *> PyMem_Malloc(sizeof (avl.avl_tree))
        self.tree = avl_new_avl_tree(avl_key_compare_for_python, < void*> self)
        if not self.tree:
            raise MemoryError("Cannot allocate tree")
        self.tree[0].root = avl.avl_new_avl_node(
            < void * > NULL, < avl.avl_node * > NULL)
    ...
    cpdef remove(self, val):
        ...
        if (avl.avl_remove_by_key(self.tree, < void * > val, avl_tree_key_free_fun) != 0):
