...
cdef class tree:
    ...
    cdef object compare_function

    def __init__(self, args=None, compare_function=None):
        self.tree = <avl.avl_tree*> malloc(sizeof(avl.avl_tree))
        # self.node_cache = avl.avl_node_new()
        # self.cache_index = 0
...
# cdef class tree(type):

#     def __init__(self, arg=None, compare_function=None):
#         self.obj = None
#         if arg is None:
#             self.obj = newavl_treeobject (compare_function)
#   if (!arg || arg == Py_None) {
 #    return (PyObject *) ;
